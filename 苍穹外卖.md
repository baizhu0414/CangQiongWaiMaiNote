# cqwm登录流程分析
    1. 登陆成功为前端生成jwt令牌，HS256 算法属于对称加密可以替换成非对称加密（如 RS256）。
    2. @ConfigurationProperties：SpringBoot配置属性类，封装配置文件中的配置。比如JwtProperties类，配置文件位于application.yml。然后将java对象通过 @Autowired注解（@Component注解结合） 注入到Controller中。
    3. 通过Lombok框架 的@Builder注解 构建对象

# 基础梳理
 ## Spring基础
**1. 基础概念**
- IoC、DI：控制反转。DI是依赖注入，属于特殊的IoC。在Spring中就是让容器控制对象间的关系。
- 面向接口变成：@Component标注在接口Service的实现类ServiceImpl上，这样容器获取接口能直接获取(AnnotationConfigApplicationContext)或注入(@Autowired)到相应对象Service obj（实际注入ServiceImpl对象）。
...
> ```java
> @Configuration
> @ComponentScan // Spring配置
> @EnableAspectJAutoProxy // AOP
> public class SpringConfig {}
> ```
> 
1. `@Component+@Aspect+@Pointcut`
    ```java
     // 表明需要对Spring bean名称进行匹配，也就是切面处理。
    ...
    ```
2. 添加切面的位置
   - @Before("pointCutMethod()") `// 此处复用了前面的切入点，而这些在同一个类中。`
   - @After
   - @AfterReturning
   - @AfterThrowing
...
- 传参方式
  ...
  - 各种传参测试：
    ...
    1）URL传参：可以通过lombok.@Data注解提供URL参数接受能力，替代Controller中函数显式传参方式。命名可以是ParamDTO这种DTO类。
    2）Body传参
    > pom.xml配置
    > 配置类函数
    ```java
